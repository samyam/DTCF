...
int main(int argc, char* argv[])
{
    ...
    int* size_B = new int[2];
    int* block_grid_B = new int[2];
    int* idmap_B = new int[2];
    ...
    Tensor* B = new Tensor("cc", idmap_B, size_B, block_grid_B, grid);
    B->initialize();
    ...
    double time =0.0;
    if(rank==0) cout<<"C [i,j,m,n ] = A [i,j,m,k ] x B [k,n ]"<<endl;
    Contraction* C0 = new Contraction(A, B, C, grid);
    time =-MPI_Wtime();
    C0->contract( "i,j,m,k", "k,n", "i,j,m,n");
