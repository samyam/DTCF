...
int main(int argc, char* argv[])
{
    ...
    int* size_C = new int[2];
    int* block_grid_C = new int[2];
    int* idmap_C = new int[2];
    ...
    Tensor* C = new Tensor("cc", idmap_C, size_C, block_grid_C, grid);
    C->initialize();

    if(rank == 0) cout<<"Tensor C initialized"<<endl;

    double time =0.0;
    if(rank==0) cout<<"C [i,m ] = A [i,k,l,h ] x B [m,k,l,h ]"<<endl;
    Contraction* C0 = new Contraction(A, B, C, grid);
    time =-MPI_Wtime();
    C0->contract( "i,k,l,h", "m,k,l,h", "i,m");
